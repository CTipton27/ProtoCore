...
def imm_to_bin(imm):
    ...
    if val > 127 or val < -128:
        ...
        val = (1<<8) + val
    return f"{val:08b}"
mem_file.write(f"memory_initialization_radix=2;\n")
mem_file.write(f"memory_initialization_vector=\n")
for line in src_file:
    ...
    instructions.append(f"{opcode}{ra_bin}{rb_bin}{rd_bin}{imm_bin}")

for i, inst in enumerate(instructions):
    if i == len(instructions) - 1:
        mem_file.write(f"{inst};\n")
    else:
        mem_file.write(f"{inst},\n")
